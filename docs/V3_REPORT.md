...
- !refilled.equals(current)는 상태가 실제로 바뀌지 않은 경우에는 CAS 자체를 수행하지 않도록 해서 불필요한 write contention과 cache invalidation을 줄이기 위함
...
Naive Atomic은 V1보다 덜 심하게 깨질 수 있으나, **정합성은 여전히 보장되지 않는다.**
